...
import {
  ...
  type Problem,
  type Shape,
} from "@/lib/quadratic-inequalities";
...
const EMPTY_ANSWER: Answer = { shape: "less-than", lo: ZERO, hi: ZERO, third: ZERO, exceptPoint: ZERO };
...
const SHAPE_LABELS: Record<Shape, string> = {
  ...
  "all-reals": "All real numbers",
  "all-except": "All reals except one point",
  "left-and-middle": "x < r₁, or r₂ < x < r₃",
  "middle-and-right": "r₁ < x < r₂, or x > r₃",
};

const SHAPES_BY_LEVEL: Record<LevelId, Shape[]> = {
  1: ["less-than", "greater-than"],
  2: ["between", "outside"],
  3: ["between", "outside", "no-solution", "all-reals", "all-except"],
  4: ["left-and-middle", "middle-and-right"],
};

type Feedback = { correct: boolean; message: string } | null;

export default function QuadraticInequalitiesPracticePanel({ level, onResult }: Props) {
  ...
  useEffect(() => {
    ...
    setAnswer({ ...EMPTY_ANSWER, shape: SHAPES_BY_LEVEL[level][0]! });
    setFeedback(null);
    setHintStage(0);
    setAiHint(null);
    setJoke(null);
  ...
  const shapeOptions = SHAPES_BY_LEVEL[level];
  ...
  async function askAiTutor() {
    ...
    try {
      const res = await fetch("/api/hint", {
        ...
        body: JSON.stringify({ poly: problem.display, level: levelDef.name, studentAttempt: formatAnswer(answer) }),
      });
  ...
  const needsLo = answer.shape === "less-than" || answer.shape === "greater-than" || answer.shape === "between" || answer.shape === "outside" || answer.shape === "left-and-middle" || answer.shape === "middle-and-right";
  const needsHi = answer.shape === "between" || answer.shape === "outside" || answer.shape === "left-and-middle" || answer.shape === "middle-and-right";
  const needsThird = answer.shape === "left-and-middle" || answer.shape === "middle-and-right";
  const needsExcept = answer.shape === "all-except";
    ...
    <div className="rounded-2xl border border-brand-line bg-brand-panel p-5">
      ...
      <div className="mt-5 flex flex-col items-center gap-3">
          ...
          {shapeOptions.map((shape) => (
            <button
              ...
              className={`rounded-lg border px-3 py-1.5 text-xs font-semibold transition disabled:opacity-40 ${
                answer.shape === shape ? "border-brand-indigo bg-brand-indigo/15 text-brand-indigo" : "border-brand-line text-brand-ink-soft hover:border-brand-indigo/50"
              }`}
            >
              {SHAPE_LABELS[shape]}
            </button>
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 text-lg">
          {needsLo && <NumberBox label="boundary 1" value={answer.lo.num} onChange={(num) => set({ lo: { num, den: 1 } })} disabled={disabled} w="w-16" />}
          {needsHi && (
            <>
              <span className="text-brand-ink-soft">,</span>
              <NumberBox label="boundary 2" value={answer.hi.num} onChange={(num) => set({ hi: { num, den: 1 } })} disabled={disabled} w="w-16" />
            </>
          )}
          {needsThird && (
            <>
              <span className="text-brand-ink-soft">,</span>
              <NumberBox label="boundary 3" value={answer.third.num} onChange={(num) => set({ third: { num, den: 1 } })} disabled={disabled} w="w-16" />
            </>
          )}
          {needsExcept && (
            <>
